...
void printReceipt(int patientID, string recID)
{
    ...
    ifstream inFileRec("./Database/Receipt/" + to_string(patientID) + recID + ".txt");
...
{
    ifstream inFile("./Page/Receipt.txt");
    ifstream inFileRec("./Database/Receipt/" + to_string(patientID) + to_string(recID) + ".txt");
...
{
    string recID = "receipt" + to_string(Rec.length() + 1);
    Rec.addLast(recID);

    string fileName = to_string(patientID) + recID + ".txt";
    string filePath = "./Database/Receipt/" + fileName;
    ofstream outFilepatient("./Database/User/Patient/patient" + to_string(patientID) + ".txt", ios::app);
