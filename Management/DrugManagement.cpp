...
{
    ifstream inFile("./Database/Drug/drugID.txt");
    string x;
...
void addDrugToDatabase(Drug &drug)
{
    ...
    string filePath = "./Database/Drug/" + fileName;
    ...
    {
        ofstream outFileDrugId("./Database/Drug/drugID.txt", ios::app);
...
void addNewDrugToDataBase()
{
    ...
    updateLine("./Database/ID.txt", 0, getLastDrugId());

    // Generate sale and write to sale file
    ofstream outFilesale("./Database/sale.txt", ios::app);
...
void deleteDrugFromDatabase(int drugID)
{
    ...
    if (status == 0)
    {
        ...
        eraseFileLine("./Database/sale.txt", drugID);
    }
    else
    {
...
LinkedList<Drug> getAllDrug()
{
    ...
    if (!inFile.is_open())
    {
        printError("Error open file drugID.txt");
    }
    string x;
...
void searchByDrugsName() {
    string name = getStringInput("Nhập tên của thuốc");
    LinkedList<Drug> drugs = searchByDrugCase(name);

    if (drugs.length()) {
        printSuccess("Tìm kiếm thành công!");
        printAllDrugs(drugs);
    } else {
...
{
    updateLine("./Database/Drug/" + to_string(id) + ".txt", 3, quantity);
}
void updateSale(int drugID, int amount)
{
    ifstream inFile("./Database/sale.txt");
    ...
    updateLine("./Database/sale.txt", line, line.substr(0, firstIndex + 1) + to_string(new_sale));
    updateLine("./Database/Drug/" + to_string(drugID) + ".txt", 5, new_sale);
}
