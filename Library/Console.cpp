...
int getDateInput(string title, int month, int year)
{
    string value;
    printLineColor(spaceLineChoice + title + ": ", 6);
    printLineColor(spaceLineChoice + "\nNgày chọn cần nằm trong khoảng từ 1 đến " + to_string(numDay(month, year)) + ": ", 6);
    getline(cin, value);
...
int getKey(int maxKey) {
    cout << endl;
    int key = getIntInput("Nhập lựa chọn");

    while (key < 1 || key > maxKey) {
        printError("Invalid key!");
        key = getIntInput("Nhập lựa chọn");
    }
    cout << endl;
    return key;
}
...
LinkedList<string> getAllEmail()
{
    ...
    ifstream inFile("./Database/User/userAccount.txt");
    string line;
    int first, last;
...
{
    ifstream inFile("./Database/ID.txt");
...
{
    ifstream inFile("./Database/Receipt/" + to_string(patientID) + recID + ".txt");
