...
{
    string recID = "receipt" + to_string(this->_Rec.length() + 1);
    this->_Rec.addLast(recID);

    string fileName = to_string(this->getID()) + recID + ".txt";
    string filePath = "./Database/Receipt/" + fileName;
    ofstream outFilePatient("./Database/User/Patient/patient" + to_string(this->getID()) + ".txt", ios::app);
...
void Patient ::bookAnAppointment()
{
    ...
    do
    {
        ...
        cout << endl;
        date = getDateInput("Bạn muốn khám bệnh ngày nào? ", month, year);
        if (isFreeDate(date, month, year) == -1)
        {
            addNewDayToDataBase(date, month, year, getID());
            printSuccess("Đặt lịch hẹn thành công!");
        ...
        {
            printError("Ngày này đã có người đặt lịch!");
        }
        choose = getYesNoInput("Bạn muốn đặt thêm lịch khám không?");
